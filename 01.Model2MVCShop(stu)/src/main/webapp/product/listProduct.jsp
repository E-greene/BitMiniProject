		...
		<td align="left">
			...
			<%} else {%>
				<% if(vo.getProTranCode().equals("0")) {%>
					판매중
				<%}else if(vo.getProTranCode().equals("1  ")) {%>
					구매완료<a href="updateTranCode.do?prodNo=<%=vo.getProdNo() %>&tranCode=2&menu=<%=menu%>&user=admin">배송하기</a>
				<%}else {%>
					배송완료
				<%} %>
			<%} %>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
	<tr>
		...
		%>
			<a href="/listProduct.do?page=<%= i%>&menu=<%=request.getAttribute("menu")%>"><%= i%></a>
		<%
			}
		%>	
